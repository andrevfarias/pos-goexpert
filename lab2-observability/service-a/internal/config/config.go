package config

import (
	"fmt"
	"log"
	"path/filepath"
	"strings"

	"github.com/spf13/viper"
)

// Config representa as configurações da aplicação
type Config struct {
	ServiceName        string `mapstructure:"SERVICE_NAME"`
	Port               string `mapstructure:"PORT"`
	ExternalAPIBaseURL string `mapstructure:"EXTERNAL_API_BASE_URL"`
	APITimeoutSeconds  int    `mapstructure:"API_TIMEOUT_SECONDS"`
	RequestNameOTEL    string `mapstructure:"REQUEST_NAME_OTEL"`
	OTELCollectorURL   string `mapstructure:"OTEL_COLLECTOR_URL"`
}

// LoadConfig carrega as configurações do arquivo .env e variáveis de ambiente
func LoadConfig(path string) (*Config, error) {
	envPath, err := filepath.Abs(path)
	if err != nil {
		return nil, fmt.Errorf("erro ao obter caminho absoluto: %w", err)
	}

	viper.AddConfigPath(envPath)
	viper.SetConfigName("app_config")
	viper.SetConfigType("env")
	viper.SetConfigFile(filepath.Join(envPath, ".env"))

	viper.SetEnvKeyReplacer(strings.NewReplacer(".", "_"))

	viper.BindEnv("SERVICE_NAME")
	viper.BindEnv("PORT")
	viper.BindEnv("EXTERNAL_API_BASE_URL")
	viper.BindEnv("API_TIMEOUT_SECONDS")
	viper.BindEnv("REQUEST_NAME_OTEL")
	viper.BindEnv("OTEL_COLLECTOR_URL")

	viper.AutomaticEnv()

	if err := viper.ReadInConfig(); err != nil {
		if _, ok := err.(viper.ConfigFileNotFoundError); !ok {
			log.Printf("Arquivo de configuração não encontrado: %v\nUtilizando apenas variáveis de ambiente", err)
		}
	}

	var config Config
	if err := viper.Unmarshal(&config); err != nil {
		return nil, fmt.Errorf("erro ao decodificar configurações: %w", err)
	}

	return &config, nil
}
